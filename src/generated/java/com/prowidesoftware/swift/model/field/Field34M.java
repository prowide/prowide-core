...
/**
 * <strong>SWIFT MT Field 34M</strong>
 * <p>
...
public class Field34M extends Field implements Serializable, AmountContainer {
    ...
    public Field34M(final Tag tag) {
        ...
        if (!StringUtils.equals(tag.getName(), "34M")) {
            throw new IllegalArgumentException("cannot create field 34M from tag "+tag.getName()+", tagname must match the name of the field.");
        }
        parse(tag.getValue());
    }
     ...
     * @see #typesPattern()
     * @return the static value of Field34M.COMPONENTS_PATTERN
     */
    @Override
    @Deprecated
    @ProwideDeprecated(phase2=TargetYear.SRU2022)
     ...
     * @since 9.2.7
     * @see #TYPES_PATTERN
     * @return the static value of Field34M.TYPES_PATTERN
     */
    @Override
     ...
     * @return the static value of Field34M.PARSER_PATTERN
     */
    @Override
    ...
    public static List<Field34M> getAll(final SwiftTagListBlock block) {
        ...
        if (arr != null && arr.length > 0) {
            final List<Field34M> result = new ArrayList<>(arr.length);
            for (final Tag f : arr) {
