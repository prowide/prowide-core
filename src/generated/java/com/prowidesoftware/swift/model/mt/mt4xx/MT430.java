...
<li class="sequence">
Sequence B - Drawee (O)<ul><li class="sequence">
Sequence B1 - Drawee (O)<ul><li class="field">Field 59 N (M)</li>
...
public class MT430 extends AbstractMT implements Serializable {
	...
	private static final java.util.logging.Logger log = java.util.logging.Logger.getLogger(MT430.class.getName());
	 ...
	 */
	public MT430(final SwiftMessage m) {
		super(m);
		sanityCheck(m);
	}
	 ...
	 * @see #MT430(String)
	 */
	public MT430(final MtSwiftMessage m) {
		this(m.message());
	}
	 ...
	 * @since 7.7
	 */
	public static MT430 parse(final MtSwiftMessage m) {
		if (m == null) {
			return null;
		}
		return new MT430(m);
    ...
    private void sanityCheck(final SwiftMessage param) {
		...
		} else if (!Strings.CS.equals(param.getType(), "430")) {
	...
	* @since 7.10.3
	*/
	public static MT430 fromJson(final String json) {
	 ...
	 * The first occurrence of field 59N at MT430 is expected to be the only one.
	 * 
	 ...
	 * or null if none is found.
	 * The first occurrence of field 59S at MT430 is expected to be the only one.
	 * 
	 ...
	 * or null if none is found.
	 * The first occurrence of field 59T at MT430 is expected to be the only one.
	 * 
	 ...
	 * or null if none is found.
	 * The first occurrence of field 59P at MT430 is expected to be the only one.
	 * 
	 ...
	 * or null if none is found.
	 * The first occurrence of field 59R at MT430 is expected to be the only one.
	 * 
	 ...
	 */
	@SequenceStyle(Type.GENERATED_SLICE) // SliceHeurisitcCodeGenerator [tagnames=[59N], type=END_OF_MESSAGE, delimiterIncludedInSequence=true ]
	...
	public SequenceB getSequenceB(SwiftTagListBlock parentSequence) {
		if (parentSequence != null && !parentSequence.isEmpty()) {
			...
			final SwiftTagListBlock content = parentSequence.getSubBlockAfterLast("59N", true);
			if (content == null) { 
				return null; 
			} 
	...
	/**
	 * Class to model Sequence "B1" in MT 430.
	 */
